 ...
 * - entryFunctionFuzzIndexes[0]: Index holding the pointer to fuzz data.
 * - entryFunctionFuzzIndexes[1]: Index holding the size of the fuzz data.
 */
...
static bool isInGlobalFileScopeList(std::string scopeName) {
  return std::find(GlobalFileScopeList.begin(), GlobalFileScopeList.end(),
                   scopeName) != GlobalFileScopeList.end();
}
...
static std::string confUpdatedFile;

static std::string createdFuncFileLock;
static std::string createdFuncFile;
...
static bool readConfiguration(Module &M) {
  if (taintFunctions.empty()) {
    ...
    if (!std::getenv("TAINT_CONFIGURATION_FILE")) {
      errs() << PRINT_RED;
      errs()
          << "[-] TAINT_CONFIGURATION_FILE environment variable is not set\n";
      errs() << PRINT_RESET;
      return false;
    }

    if (!std::getenv("TAINT_CONFIGURATION_FILE_LOCK")) {
      errs() << PRINT_RED;
      errs() << "[-] TAINT_CONFIGURATION_FILE_LOCK environment variable is not "
                "set\n";
      errs() << PRINT_RESET;
      return false;
    }

    if (!std::getenv("TAINT_UPDATE_FUNC_FILE")) {
      errs() << PRINT_RED;
      errs() << "[-] TAINT_UPDATE_FUNC_FILE environment variable is not set\n";
      errs() << PRINT_RESET;
      return false;
    }

    if (!std::getenv("CREATE_FUNC_FILE")) {
      errs() << PRINT_RED;
      errs() << "[-] CREATE_FUNC_FILE environment variable is not set\n";
      errs() << PRINT_RESET;
      return false;
    }

    if (!std::getenv("CREATE_FUNC_FILE_LOCK")) {
      errs() << PRINT_RED;
      errs() << "[-] CREATE_FUNC_FILE_LOCK environment variable is not set\n";
      errs() << PRINT_RESET;
      return false;
    }
    ...
    lockFile = std::getenv("TAINT_CONFIGURATION_FILE_LOCK");
    confUpdatedFile = std::getenv("TAINT_UPDATE_FUNC_FILE");

    createdFuncFile = std::getenv("CREATE_FUNC_FILE");
    createdFuncFileLock = std::getenv("CREATE_FUNC_FILE_LOCK");

    int fd = open(lockFile.c_str(), O_CREAT | O_RDWR, 0666);
    if (fd < 0) {
      errs() << PRINT_RED;
      errs() << "[-] Failed to open lock file\n";
      errs() << PRINT_RESET;
      return false;
    }
    ...
    if (config["entry"].has_key()) {
      ...
      if (entryFunction && !isInTaintFunctionsList(entryFunction) &&
        ...
        config["entry"]["input_index"] >> input_index;
        config["entry"]["length_index"] >> len_index;

        if (input_index >= (ssize_t)entryFunction->arg_size() ||
            len_index >= (ssize_t)entryFunction->arg_size()) {
          errs() << PRINT_RED;
          errs() << "[-] Invalid taint index for " << entryFunctionName << "\n";
          ...
          /* There could be other fit. Give a second chance. 
           * Of course, if there is no entryfunction across the project,
           * the building would be failed. */
          entryFunctionName = "";
          // return false;
        } else {
          for (int i = 0; i < entryFunction->arg_size(); i++) {
            if (i == input_index || i == len_index) {
              continue;
            }
          ...
          entryFunctionFuzzIndexes.push_back(input_index);
          entryFunctionFuzzIndexes.push_back(len_index);
        }
      }
    } else {
      errs() << PRINT_RED;
...
static bool insertTaintConfiguration(std::string funcName, bool isTainted) {
  ...
  std::ofstream outUpdated(confUpdatedFile);
  outUpdated << funcName << "\n";
...
static bool updateTaintReturnConfiguration(Function &F,
  ...
  std::ofstream outUpdated(confUpdatedFile, std::ios_base::app);
  std::string fileName = F.getParent()->getSourceFileName();
  outUpdated << fileName << "," << funcName << "\n";
...
static void insertCreatedFunction(Function *F, std::string funcName) {
  int fd = open(createdFuncFileLock.c_str(), O_CREAT | O_RDWR, 0666);
  if (fd < 0) {
    errs() << PRINT_RED;
    errs() << "[-] Failed to open lock file\n";
    errs() << PRINT_RESET;
...
std::map<Function *, std::map<Value *, int>> taintScore;
...
static void addTaintValue(Function *F, Value *V, int score) {
  if (taintMap.find(F) == taintMap.end()) {
    taintMap[F] = std::vector<Value *>();
    taintScore[F] = std::map<Value *, int>();
  }

  if (std::find(taintMap[F].begin(), taintMap[F].end(), V) ==
      taintMap[F].end()) {
    taintMap[F].push_back(V);
    taintScore[F][V] = score;
  } else {
    /* There is previous score */
    if (taintScore[F][V] < score) {
      taintScore[F][V] = score;
    }
  }
}
...
static int getTaintScore(Function *F, Value *V) {
  /* Tainted by global variable */
  if (std::find(taintMap[nullptr].begin(), taintMap[nullptr].end(), V) !=
      taintMap[nullptr].end()) {
    return 100;
  ...
  if (taintScore.find(F) == taintScore.end()) {
    return 0;
  }

  if (taintScore[F].find(V) == taintScore[F].end()) {
    return 0;
  }

  return taintScore[F][V];
}
...
static void singleLineTaintFlow(Module &M, Function &F, Instruction &I) {
  ...
  if (auto *Store = dyn_cast<StoreInst>(&I)) {
    ...
    if (isTaintValue(&F, from) && !isTaintValue(&F, to)) {
      /* A' -> B */
      addTaintValue(&F, to, getTaintScore(&F, from));
      addTaintValue(&F, Store, getTaintScore(&F, from));
      tainted = true;
      if (auto *GEP = dyn_cast<GetElementPtrInst>(to)) {
        ...
        if (!isTaintValue(&F, GEPFrom)) {
          ...
          addTaintValue(&F, GEPFrom, getTaintScore(&F, from));
        }
      }
    } else if (isTaintValue(&F, from) && isTaintValue(&F, to)) {
      /* A' -> B' */
      int largeScore = std::max(getTaintScore(&F, from), getTaintScore(&F, to));
      addTaintValue(&F, to, largeScore);
      addTaintValue(&F, Store, largeScore);
      tainted = true;
    } else if (!isTaintValue(&F, from) && isTaintValue(&F, to)) {
      /* A -> B' */
      addTaintValue(&F, Store, getTaintScore(&F, to));
      tainted = true;
    }
    ...
    if (isTaintValue(&F, from)) {
      ...
      /* A + B' = C' */
      addTaintValue(&F, to, getTaintScore(&F, offset) * 0.5);
      tainted = true;
    }
    ...
    for (Value *arg : Phi->incoming_values()) {
      if (isTaintValue(&F, arg)) {
        total_score += getTaintScore(&F, arg);
        tainted = true;
      }
    }
    if (tainted) {
      addTaintValue(&F, Phi, total_score / Phi->getNumOperands());
    }
  } else if (auto *Binary = dyn_cast<BinaryOperator>(&I)) {
    for (Value *operand : Binary->operands()) {
      if (isTaintValue(&F, operand)) {
        addTaintValue(&F, Binary, getTaintScore(&F, operand));
        tainted = true;
      }
    }
    ...
    if (callee && isInGlobalScopeList(callee->getName().str()) &&
      ...
      if (isFunctionAnalyzed(funcName)) {
        ...
        if (taintIndices) {
          for (int index : *taintIndices) {
            addTaintValue(&F, Call->getArgOperand(index),
                          getTaintScore(&F, Call->getArgOperand(index)));
          }
        }
      } else {
        ...
        /* Taint all arguments */
        for (int i = 0; i < Call->arg_size(); i++) {
          addTaintValue(&F, Call->getArgOperand(i), 100);
        }
        tainted = true;
      }
    } else {
...
static void singleFunctionTaintFlow(Module &M, Function &F,
  ...
  for (int index : *taintIndices) {
    addTaintValue(&F, F.getArg(index),
                  getTaintScore(&F, F.getArg(index)) > 0
                      ? getTaintScore(&F, F.getArg(index))
                      : 100);
#ifdef TAINT_DEBUG
    std::cerr << PRINT_GREEN;
...
static CallInst *insertSanitizeLoad(Function &F, LoadInst *Inst,
  ...
  addTaintValue(&F, Call, getTaintScore(&F, Inst->getPointerOperand()));
...
static CallInst *insertSanitizeSwitch(Function &F, SwitchInst *Inst,
  ...
  /* Created call should be tainted */
  addTaintValue(&F, Call, getTaintScore(&F, from));
...
#ifdef SIMPLE_DFSAN
  /* Record taint score for each arguments */
  Function *recordFunc = M.getFunction("recordSanitizeCallTaintScore");
  if (!recordFunc) {
    FunctionType *recordFuncType =
        FunctionType::get(Type::getVoidTy(M.getContext()),
                          {Builder.getInt32Ty(), Builder.getInt32Ty()}, false);
    recordFunc = Function::Create(recordFuncType, Function::ExternalLinkage,
                                  "recordSanitizeCallTaintScore", M);
  }

  for (int i = 0; i < Inst->arg_size(); i++) {
    Value *arg = Inst->getArgOperand(i);
    CallInst *recordCall = Builder.CreateCall(
        recordFunc,
        {Builder.getInt32(i), Builder.getInt32(getTaintScore(&F, arg))});
  }
#endif
...
insertIndirectLinkCall(Function &F, CallInst *Inst,
  ...
  for (std::string &IndirectLink : IndirectLinks) {
    ...
    if (!CheckFunc) {
      FunctionType *checkFuncType =
          FunctionType::get(Type::getInt1Ty(C), {Builder.getPtrTy()}, false);
      CheckFunc = Function::Create(checkFuncType, GlobalValue::ExternalLinkage,
                                   checkFuncName,
                                   F.getParent());
    }
    CallInst *CheckCall =
        Builder.CreateCall(CheckFunc, {insertBefore->getCalledOperand()});
      ...
      if (!Call->isIndirectCall() && Call->getCalledFunction()) {
        ...
        if (!isInGlobalScopeList(Call->getCalledFunction()->getName().str()) ||
          ...
          FunctionType *sanitizeFuncType = FunctionType::get(
              Call->getType(), {Call->getType()}, false);
          ...
          Function *sanitizeFunc = M.getFunction("sanitizeCall" + sanitizeType);
          if (!sanitizeFunc) {
            sanitizeFunc =
          ...
          std::cerr << PRINT_RESET;
#endif
          insertSanitizeDirectCall(F, Call, sanitizeFunc);
#ifdef SANITIZE_DEBUG
          std::cerr << PRINT_CYAN << "+ ";
          Call->print(errs());
          errs() << "\n";
        ...
        FunctionType *sanitizeFuncType = FunctionType::get(
            Call->getType(), {Type::getVoidTy(M.getContext())}, false);
        std::string sanitizeType;
        if (Call->getType()->isIntegerTy()) {
          sanitizeType =
              "13_" + std::to_string(Call->getType()->getIntegerBitWidth());
...
static void createEntryFunction(Module &M) {
  ...
  FunctionType *entryFunctionType = FunctionType::get(
      Type::getVoidTy(C), {Builder.getPtrTy(), Builder.getInt32Ty()}, false);
  ...
  std::vector<Value *> args;
  for (int i = 0; i < F->arg_size(); i++) {
    if (i == entryFunctionFuzzIndexes[0]) {
      /* Fuzz data */
      args.push_back(entryFunction->getArg(0));
    } else if (i == entryFunctionFuzzIndexes[1]) {
      /* Fuzz data length*/
      args.push_back(entryFunction->getArg(1));
    } else {
      /* Otherwise, just push zero */
  ...
  FunctionType *crossEntryPointsType = FunctionType::get(
    Type::getInt1Ty(C), {Builder.getPtrTy(), Builder.getInt32Ty()}, false);
...
/* Enum values bitmap */
enum TaintOriginIndex {
  TaintOriginIndex0 = 1 << 0,
  TaintOriginIndex1 = 1 << 1,
  TaintOriginIndex2 = 1 << 2,
  TaintOriginIndex3 = 1 << 3,
  TaintOriginIndex4 = 1 << 4,
  TaintOriginIndex5 = 1 << 5,
  TaintOriginIndex6 = 1 << 6,
  TaintOriginIndex7 = 1 << 7,
  TaintOriginIndex8 = 1 << 8,
  TaintOriginIndex9 = 1 << 9,
  TaintOriginIndex10 = 1 << 10,
  TaintOriginIndex11 = 1 << 11,
  TaintOriginIndex12 = 1 << 12,
  TaintOriginIndex13 = 1 << 13,
  TaintOriginConstant = 1 << 14,
  TaintOriginGlobal = 1 << 15,
};

std::map<Function *, std::map<Value *, uint16_t>> SimpleDFSANtaintMap;

static void SimpleDFSANUpdateTaintValue(Function *F, Value *V, uint16_t index) {
  if (SimpleDFSANtaintMap.find(F) == SimpleDFSANtaintMap.end()) {
    SimpleDFSANtaintMap[F] = std::map<Value *, uint16_t>();
  }

  if (SimpleDFSANtaintMap[F].find(V) == SimpleDFSANtaintMap[F].end()) {
    SimpleDFSANtaintMap[F][V] = index;
  } else {
    SimpleDFSANtaintMap[F][V] |= index;
  }
}

static uint16_t SimpleDFSANGetTaintValue(Function *F, Value *V) {
  if (SimpleDFSANtaintMap.find(F) == SimpleDFSANtaintMap.end()) {
#ifdef SIMPLE_DFSAN_DEBUG
    errs() << "DFSAN: taint value not found: ";
    V->print(errs());
    errs() << "\n";
#endif
    return TaintOriginConstant;
  }
  ...
  return SimpleDFSANtaintMap[F][V];
}

static void SimpleDFSANInsertHook(Function *F, Instruction *I, Function *Hook,
                                  Value *addr) {
  LLVMContext &C = F->getContext();
  IRBuilder<> Builder(C);
  ...
  CallInst *Call = Builder.CreateCall(
      Hook, {ConstantInt::get(Type::getInt16Ty(C),
                              SimpleDFSANGetTaintValue(F, I) & 0xFFFF),
             addr});

  /* Created call should be tainted */
  SimpleDFSANUpdateTaintValue(F, Call, SimpleDFSANGetTaintValue(F, I));
}

static void SimpleDFSANFlowTaintValue(Function *F, Value *From, Value *To) {
  /* Check From Value is global variable */
  if (isTaintValue(nullptr, From)) {
    SimpleDFSANUpdateTaintValue(F, From, TaintOriginGlobal);
  } else if (isTaintValue(nullptr, To)) {
    SimpleDFSANUpdateTaintValue(F, To, TaintOriginGlobal);
  }

  SimpleDFSANUpdateTaintValue(F, To, SimpleDFSANGetTaintValue(F, From));
}

static void SimpleDFSANInstruction(Function *F, Instruction &I) {
  if (auto *Store = dyn_cast<StoreInst>(&I)) {
    Value *from = Store->getValueOperand();
    Value *to = Store->getPointerOperand();

    SimpleDFSANFlowTaintValue(F, from, to);

    /* DFSAN Store Hook */
    Function *DFSANStoreHook = F->getParent()->getFunction("DFSANStoreHook");
    if (!DFSANStoreHook) {
      DFSANStoreHook = Function::Create(
          FunctionType::get(to->getType(),
                            {Type::getInt16Ty(F->getContext()), to->getType()},
                            false),
          Function::ExternalLinkage, "DFSANStoreHook", F->getParent());
    }
    SimpleDFSANInsertHook(F, Store, DFSANStoreHook, to);
  } else if (auto *Load = dyn_cast<LoadInst>(&I)) {
    Value *from = Load->getPointerOperand();
    Value *to = Load;
    ...
    /* DFSAN Load Hook */
    Function *DFSANLoadHook = F->getParent()->getFunction("DFSANLoadHook");
    if (!DFSANLoadHook) {
      DFSANLoadHook = Function::Create(
          FunctionType::get(
              to->getType(),
              {Type::getInt16Ty(F->getContext()), from->getType()}, false),
          Function::ExternalLinkage, "DFSANLoadHook", F->getParent());
    }
    SimpleDFSANInsertHook(F, Load, DFSANLoadHook, from);
  } else if (auto *GetElementPtr = dyn_cast<GetElementPtrInst>(&I)) {
    Value *from = GetElementPtr->getPointerOperand();
    Value *offset = GetElementPtr->getOperand(1);
    Value *to = GetElementPtr;
    ...
    SimpleDFSANFlowTaintValue(F, from, to);
    SimpleDFSANFlowTaintValue(F, offset, to);
  } else if (auto *Phi = dyn_cast<PHINode>(&I)) {
    for (Value *arg : Phi->incoming_values()) {
      SimpleDFSANFlowTaintValue(F, arg, Phi);
    }
  } else if (auto *Binary = dyn_cast<BinaryOperator>(&I)) {
    Value *from = Binary->getOperand(0);
    Value *to = Binary;
    ...
    SimpleDFSANFlowTaintValue(F, op1, Cmp);
    SimpleDFSANFlowTaintValue(F, op2, Cmp);
  } else if (auto *Call = dyn_cast<CallInst>(&I)) {
    /* We do not need to propagate tainting when there is no return value */
    ...
    for (int i = 0; i < Call->arg_size(); i++) {
      ...
      SimpleDFSANFlowTaintValue(F, arg, Call);
    }
  }
}

static void SimpleDFSANModule(Module &M) {
  for (Function &F : M) {
    if (isInTaintFunctionsList(&F) || F.isDeclaration() || F.isIntrinsic()) {
      continue;
    }

    if (F.hasName() && ((F.getName().str() == "DFSANStoreHook") ||
                        (F.getName().str() == "DFSANLoadHook") ||
                        (F.getName().str() == "__asan_on_error_report"))) {
      continue;
    }
    /* Taint function arguments with its indexes */
    for (int i = 0; i < F.arg_size(); i++) {
      SimpleDFSANUpdateTaintValue(&F, F.getArg(i), 1 << i);
    }

    for (BasicBlock &BB : F) {
      for (Instruction &I : BB) {
        SimpleDFSANInstruction(&F, I);
      }
    }
  }
}
...
#ifdef SIMPLE_DFSAN
struct SimpleDFSAN : public PassInfoMixin<SimpleDFSAN> {
  ...
  const auto callback = [](PassBuilder &PB) {
    ...
    PB.registerOptimizerEarlyEPCallback(
        [](ModulePassManager &MPM, OptimizationLevel level) {
          ...
          // MPM.addPass(TargetEntryFunctionGenerator());
#ifdef SIMPLE_DFSAN
      // MPM.addPass(SimpleDFSAN());
#endif
        });
  };
