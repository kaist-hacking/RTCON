...
// Deprecated
/*
extern int topFuzzer;
__attribute__((no_sanitize("coverage"), no_sanitize("address"))) static void
dumpSanitizedScope(char *name) {
  // We only dump the sanitized scope for top fuzzer
  if (topFuzzer == 0) {
    return;
  }

  char sanitized_scope_file_full_path[200];
  snprintf(sanitized_scope_file_full_path, 200, "crash_dir/sanitized_scope.txt");

  FILE *sanitized_scope_file = fopen(sanitized_scope_file_full_path, "a");
  if (sanitized_scope_file == NULL) {
    printf(RED "Error creating sanitized scope file" RESET "\n");
    exit(0);
  }

  fprintf(sanitized_scope_file, "%s\n", name);
  fclose(sanitized_scope_file);
}
*/
