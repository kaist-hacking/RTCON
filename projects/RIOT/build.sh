...
if [ ! -f "/configs/riot-func-list-reduced.txt" ]; then
    echo "[INFO] Reducing the function list"
    python3 /source/cfg/config_reducer.py --dot tcg.dot --ll whole_project.ll --target /configs/riot-full-func-list.txt --output /configs/riot-func-list-reduced.txt
fi

echo "[INFO] Analyzing CFG for ${FUNC_NAME}"
python3 /source/cfg/analyze_cfg.py --dot tcg.dot --ll whole_project.ll --target /configs/riot-func-list-reduced.txt --function ${FUNC_NAME} --file ${FILE_PATH}
