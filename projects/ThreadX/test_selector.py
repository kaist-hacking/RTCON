...
def replace_test_func(choice):
    ...
    main_file = os.path.join(source_dir, "fuzz", "src", "main.c")
    main_cov_file = os.path.join(source_dir, "fuzz", "src", "main_cov.c")
    ...
    # replace main_cov file
    if os.path.exists(main_cov_file):
        with open(main_cov_file, "rt") as f:
            content = f.read()
        with open(main_cov_file, "wt") as f:
            f.write(content.replace("testRawData(Data, Size);", f"{func_dict[choice]}(Data, Size);"))
