...
/* ThreadX impelment 32bit str functions */
char *strstr_wrapper(char *haystack, char *needle) {
  char *ret_32bit = strstr(haystack, needle);
  if (ret_32bit == NULL) {
    return NULL;
  }
  uint32_t diff = ((uint64_t)ret_32bit & 0xffffffff) - ((uint64_t)haystack & 0xffffffff);
  return haystack + diff;
}

char *strchr_wrapper(char *s, int c) {
  char *ret_32bit = strchr(s, c);
  if (ret_32bit == NULL) {
    return NULL;
  }
  uint32_t diff = ((uint64_t)ret_32bit & 0xffffffff) - ((uint64_t)s & 0xffffffff);
  return s + diff;
}
...
getCrashAddress(const char *report, int index) {
  ...
  char *pc = strstr_wrapper(report, index_str);
  if (pc == NULL) {
    return NULL;
  }
  pc += strlen(index_str);
...
getCrashFunctionName(const char *report, char *function_name, int len) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (pc != NULL) {
      pc += strlen(index_str);
      if (strstr_wrapper(pc, "in ") != NULL) {
        pc = strstr_wrapper(pc, "in ") + 3;
        char *bracket_end = strchr_wrapper(pc, '(');
        char *space_end = strchr_wrapper(pc, ' ');
        ...
        if (memcmp(pc, "replicate_", 10) == 0) {
          ...
          char *line_end = strchr_wrapper(pc, '\n');
          char *header = strstr_wrapper(pc, ".h");
...
isInFuzzScope(const char *report) {
  ...
  for (i = 0; i < size; i++) {
    if (strstr_wrapper(strings[i], "fuzzEntryFunction") != NULL) {
      free(strings);
      return true;
    }
  }
  ...
  if (strstr_wrapper(report, "fuzzEntryFunction") != NULL) {
    return true;
  }

  /* IndirectLink lose the trace */
  if (strstr_wrapper(report, "callIndirectLink") != NULL) {
    return true;
  }
...
isInUntrackableFunction(const char *report) {
  ...
  for (i = 0; i < size; i++) {
    for (unsigned int j = 0; j < sizeof(untrackableFunctions) / sizeof(char *);
         j++) {
      if (strstr_wrapper(strings[i], untrackableFunctions[j]) != NULL) {
        free(strings);
        return true;
      }
    }
  ...
  for (unsigned int j = 0; j < sizeof(untrackableFunctions) / sizeof(char *);
       j++) {
    if (strstr_wrapper(report, untrackableFunctions[j]) != NULL) {
      return true;
    }
  }
...
handleHighConfidentialityCrash(const char *report) {
  ...
  int index = 0;
  while (1) {
    // We only care about the first 3 crash addresses
    if (index > 2) break;
