...
BUILD_DIR = ""
BUILD_COMMAND = (
    "rm -rf CMakeCache.txt CMakeFiles && mkdir -p %s && cd %s && cmake -GNinja -DCRASH_DIR_PATH=\"%s\" -DFUZZ=on .. && ninja -j 1"
)
BUILD_CLEAN_COMMAND = "rm -rf %s"
...
def get_nprocs():
    # ThreadX only supports 1 thread
...
def build_single_fuzzer(func_name):
    ...
    while True:
        ...
        try:
            p = subprocess.Popen(
                BUILD_COMMAND % (build_dir, build_dir, crash_dir),
                env=fuzzer_env,
                shell=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            ...
            if first_build:
                ...
                if os.path.exists(taint_update_func_file):
                    ...
                    shutil.copy(f"{build_dir}/src/threadx-fuzz", f"./host_bin/threadx-{func_name}-fuzz")
                    break
