...
BUILD_COMMAND = (
    "rm /tmp/%s_multi_entry.txt; ccache -C && west build -p always -d %s -o=-j1 -b native_posix/native/64 -- -DFUZZ=on ."
    # "rm /tmp/%s_multi_entry.txt; ccache -C && west build -p always -d %s -b native_posix/native/64 -- -DFUZZ=on ." # Buggy
)
BUILD_CLEAN_COMMAND = ""
...
TEST_FUNC_LIST = "/configs/zephyr-func-list-reduced.txt"
...
def build_single_fuzzer(func_name):
    ...
    build_dir = os.path.abspath(f"build/build_{func_name}")
    ...
    while True:
        ...
        try:
            p = subprocess.Popen(
                BUILD_COMMAND % (func_name, build_dir),
                env=fuzzer_env,
                shell=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            ...
            if first_build:
                ...
                if os.path.exists(taint_update_func_file):
                    ...
                    shutil.copy(f"{build_dir}/zephyr/zephyr.elf", f"./host_bin/zephyr-{func_name}-fuzz")
                    # shutil.copy(f"{build_dir}/zephyr/zephyr.elf", f"./host_bin/topzephyr-{func_name}-fuzz")
                    break
